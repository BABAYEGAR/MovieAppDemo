    ...
    Mapael.prototype = {
        ...
        init: function() {
            ...
            if (self.options.map.width) {
                ...
                self.paper.setSize(self.options.map.width,
            ...
            // Draw plots
            $.each(self.options.plots,
                function(id) {
                    self.plots[id] = self.drawPlot(id);
                });

            // Attach zoom event
            self.$container.on("zoom." + pluginName,
                function(e, zoomOptions) {
            ...
            // Attach update event
            self.$container.on("update." + pluginName,
                function(e, opt) {
                    self.onUpdateEvent(e, opt);
                });

            // Attach showElementsInRange event
            self.$container.on("showElementsInRange." + pluginName,
                function(e, opt) {
        ...
        handleMapResizing: function() {
            ...
            self.$map.on("resizeEnd." + pluginName,
                function() {
                ...
                function(type) {
                    if ($.isArray(options.legend[type])) {
                        for (var i = 0; i < options.legend[type].length; ++i)
                            options.legend[type][i] = $.extend(true,
                                {},
                                Mapael.prototype.legendDefaultOptions[type],
                                options.legend[type][i]);
                    } else {
                        options.legend[type] = $.extend(true,
                            {},
                            Mapael.prototype.legendDefaultOptions[type],
                            options.legend[type]);
                    }
        ...
        initElem: function(elem, elemOptions, id) {
            ...
            if (elemOptions.text && elemOptions.text.content !== undefined) {
                ...
                elem.textElem = self.paper.text(textPosition.x, textPosition.y, elemOptions.text.content)
                    .attr(elemOptions.text.attrs);
        ...
        initZoom: function(mapWidth, mapHeight, zoomOptions) {
            ...
            if (self.options.map.zoom.mousewheel) {
                self.$map.on("mousewheel." + pluginName,
                    function(e) {
            ...
            if (self.options.map.zoom.touch) {
                self.$map.on("touchstart." + pluginName,
                    function(e) {
                ...
                self.$map.on("touchmove." + pluginName,
                    function(e) {
                        var pinchDist = 0;
                        var zoomLevel = 0;
                        ...
                        if (e.originalEvent.touches.length === 2) {
                            ...
                            if (Math.abs(pinchDist - self.previousPinchDist) > 15) {
                                ...
                                zoomLevel = (pinchDist - self.previousPinchDist) /
            ...
            // Panning
            $("body").on("mouseup." + pluginName + (zoomOptions.touch ? " touchend" : ""),
                function() {
                    mousedown = false;
                    setTimeout(function() {
                            self.panning = false;
            ...
            self.$map.on("mousedown." + pluginName + (zoomOptions.touch ? " touchstart" : ""),
                function(e) {
                ...
                }).on("mousemove." + pluginName + (zoomOptions.touch ? " touchmove" : ""),
                function(e) {
                    ...
                    if (mousedown && currentLevel !== 0) {
                        var offsetX = (previousX - pageX) /
                            (1 + (currentLevel * zoomOptions.step)) *
                            (mapWidth / self.paper.width);
                        var offsetY = (previousY - pageY) /
                            (1 + (currentLevel * zoomOptions.step)) *
                            (mapHeight / self.paper.height);
                        var panX = Math.min(Math.max(0, self.paper._viewBox[0] + offsetX),
                            (mapWidth - self.paper._viewBox[2]));
                        var panY = Math.min(Math.max(0, self.paper._viewBox[1] + offsetY),
                            (mapHeight - self.paper._viewBox[3]));
        ...
        onZoomEvent: function(e, zoomOptions) {
            ...
            var zoomLevel = 0;
            var animDuration = (zoomOptions.animDuration !== undefined)
            ...
            if (newLevel === 0) {
                ...
                offsetX = self.zoomData.panX +
                    ((zoomOptions.x - self.zoomData.panX) * (zoomLevel - previousZoomLevel)) / zoomLevel;
                offsetY = self.zoomData.panY +
                    ((zoomOptions.y - self.zoomData.panY) * (zoomLevel - previousZoomLevel)) / zoomLevel;
                ...
                panX = Math.min(Math.max(0, zoomOptions.x - (self.mapConf.width / zoomLevel) / 2),
                    (self.mapConf.width - (self.mapConf.width / zoomLevel)));
                panY = Math.min(Math.max(0, zoomOptions.y - (self.mapConf.height / zoomLevel) / 2),
                    (self.mapConf.height - (self.mapConf.height / zoomLevel)));
            }
            ...
            if (animDuration > 0) {
                self.animateViewBox(panX,
                    panY,
                    self.mapConf.width / zoomLevel,
                    self.mapConf.height / zoomLevel,
                    animDuration,
        ...
        setElementOpacity: function(elem, opacity, animDuration) {
            ...
            if (animDuration > 0) {
                // Animate attribute
                elem.mapElem.animate({ "opacity": opacity },
                    animDuration,
                    "linear",
                    function() {
        ...
        onUpdateEvent: function(e, opt) {
            ...
            var fnRemoveElement = function(elem) {
                ...
                if (animDuration > 0) {
                    elem.mapElem.animate({ "opacity": 0 },
                        animDuration,
                        "linear",
                        function() {
                            elem.mapElem.remove();
                    ...
                    if (elem.textElem) {
                        elem.textElem.animate({ "opacity": 0 },
                            animDuration,
                            "linear",
                            function() {
            ...
            if (typeof opt.mapOptions === "object") {
                ...
                if (opt.mapOptions.areas !== undefined ||
                ...
                function(id) {
                    ...
                    if ((typeof opt.mapOptions === "object" &&
                        ...
                        if (elemOptions.type == "square") {
                            ...
                            elemOptions.attrs.x = self.plots[id].mapElem.attrs.x -
                                (elemOptions.size - self.plots[id].mapElem.attrs.width) / 2;
                            elemOptions.attrs.y = self.plots[id].mapElem.attrs.y -
                                (elemOptions.size - self.plots[id].mapElem.attrs.height) / 2;
                        } else if (elemOptions.type == "image") {
                            elemOptions.attrs.width = elemOptions.width;
                            elemOptions.attrs.height = elemOptions.height;
                            elemOptions.attrs.x = self.plots[id].mapElem.attrs.x -
                                (elemOptions.width - self.plots[id].mapElem.attrs.width) / 2;
                            elemOptions.attrs.y = self.plots[id].mapElem.attrs.y -
                                (elemOptions.height - self.plots[id].mapElem.attrs.height) / 2;
                        } else if (elemOptions.type == "svg") {
                            if (elemOptions.attrs.transform !== undefined) {
                                elemOptions.attrs.transform =
                                    self.plots[id].mapElem.baseTransform + elemOptions.attrs.transform;
                            }
                        } else { // Default : circle
        ...
        updateElem: function(elemOptions, elem, animDuration) {
            ...
            // Update the label
            if (elem.textElem) {
                if (elemOptions.text !== undefined &&
                    elemOptions.text.content !== undefined &&
            ...
            if (elemOptions.type == "svg") {
                elem.mapElem.transform("m" +
                    (elemOptions.width / elem.mapElem.originalWidth) +
                    ",0,0," +
                    (elemOptions.height / elem.mapElem.originalHeight) +
                    "," +
                    bbox.x +
                    "," +
                    bbox.y);
        ...
        drawPlot: function(id) {
            ...
            if (elemOptions.type == "square") {
                ...
                plot.mapElem.baseTransform = "m" +
                    (elemOptions.width / plot.mapElem.originalWidth) +
                    ",0,0," +
                    (elemOptions.height / plot.mapElem.originalHeight) +
                    "," +
                    (coords.x - elemOptions.width / 2) +
                    "," +
                    (coords.y - elemOptions.height / 2);
        ...
        setTooltip: function(elem) {
            ...
            var updateTooltipPosition = function(x, y) {
                ...
                var tooltipPosition = {
                    "left": Math.min(self.$map.width() - self.$tooltip.outerWidth() - 5,
                        x - self.$map.offset().left + offsetLeft),
                    "top": Math.min(self.$map.height() - self.$tooltip.outerHeight() - 5,
                        y - self.$map.offset().top + offsetTop)
                };
            ...
            $(elem.node).on("mouseover." + pluginName,
                function(e) {
                        ...
                        function() {
                            ...
                            if (elem.tooltip !== undefined) {
                                if (elem.tooltip.content !== undefined) {
                                    ...
                                    var content = (typeof elem.tooltip.content === "function")
                                        ? elem.tooltip.content(elem)
                                        : elem.tooltip.content;
        ...
        setEventHandlers: function(id, elemOptions, mapElem, textElem) {
            var self = this;
            $.each(elemOptions.eventHandlers,
                function(event) {
                    (function(event) {
                        $(mapElem.node).on(event,
                            function(e) {
                                if (!self.panning)
                                    elemOptions.eventHandlers[event](e, id, mapElem, textElem, elemOptions);
                            });
                        if (textElem) {
                ...
                if (sliceOptions[i].display === undefined || sliceOptions[i].display === true) {
                    if (legendType == "area") {
                        ...
                        if (sliceOptions[i].type == "image") {
                            ...
                            sliceOptions[i].attrs.transform = "m" +
                                ((scale * sliceOptions[i].width) / elem.getBBox().width) +
                                ",0,0," +
                                ((scale * sliceOptions[i].height) / elem.getBBox().height) +
                                "," +
                                x +
                                "," +
                                y +
        ...
        handleClickOnLegendElem: function(legendOptions, sliceOptions, label, elem, elems, legendIndex) {
            ...
            var hideMapElems = function(e, hideOtherElems, animDuration) {
                        ...
                        if ((sliceOptions.sliceValue !== undefined && elemValue == sliceOptions.sliceValue) ||
                            ...
                            (function(id) {
                                if (hidden === "0") { // we want to hide this element
                                    ...
                                    self.setElementOpacity(elems[id],
                                    ...
                                    if ($.isEmptyObject(hiddenBy)) {
                                        self.setElementOpacity(
                                            elems[id],
                                            elems[id].mapElem.originalAttrs.opacity !== undefined
                ...
                if ((hideOtherElems === undefined || hideOtherElems === true) &&
                    legendOptions.exclusive !== undefined &&
        ...
        createLegends: function(legendType, elems, scale) {
            ...
            for (var j = 0; j < legendsOptions.length; ++j) {
                ...
                if (legendsOptions[j].display === true &&
        ...
        elemHover: function(mapElem, textElem) {
            ...
            // Set textElem
            if (textElem) {
                if (textElem.attrsHover.animDuration > 0)
                    textElem.animate(textElem.attrsHover, textElem.attrsHover.animDuration);
        ...
        elemOut: function(mapElem, textElem) {
            ...
            if (textElem) {
                if (textElem.attrsHover.animDuration > 0)
                    textElem.animate(textElem.originalAttrs, textElem.attrsHover.animDuration);

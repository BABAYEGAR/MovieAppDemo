(function(window) {
    ...
    c3_chart_internal_fn.initParams = function() {
        ...
        $$.yOrient =
            config.axis_rotated ? (config.axis_y_inner ? "top" : "bottom") : (config.axis_y_inner ? "right" : "left");
        $$.y2Orient = config.axis_rotated
            ? (config.axis_y2_inner ? "bottom" : "top")
            : (config.axis_y2_inner ? "left" : "right");
    ...
    c3_chart_internal_fn.initWithData = function(data) {
        ...
        main.append("text")
            ...
            .attr("dominant-baseline",
                "middle"); // vertical centering of text at y position in all browsers, except IE.
    ...
    c3_chart_internal_fn.updateSizes = function() {
        ...
        $$.width2 =
            config.axis_rotated ? $$.margin.left - $$.rotated_padding_left - $$.rotated_padding_right : $$.width;
    ...
    c3_chart_internal_fn.updateScales = function() {
        ...
        $$.x = $$.getX($$.xMin,
            ...
            function() { return $$.xAxis.tickOffset(); });
        $$.y = $$.getY($$.yMin, $$.yMax, forInit ? config.axis_y_default : $$.y.domain());
        $$.y2 = $$.getY($$.yMin, $$.yMax, forInit ? config.axis_y2_default : $$.y2.domain());
        $$.subX =
            $$.getX($$.xMin, $$.xMax, $$.orgXDomain, function(d) { return d % 1 ? 0 : $$.subXAxis.tickOffset(); });
    ...
    c3_chart_internal_fn.mapTargetsToUniqueXs = function(targets) {
        var $$ = this;
        var xs = $$.d3.set($$.d3.merge(targets.map(function(t) { return t.values.map(function(v) { return +v.x; }); })))
            .values();
    ...
    c3_chart_internal_fn.filterByX = function(targets, x) {
        return this.d3.merge(targets.map(function(t) { return t.values; }))
            .filter(function(v) { return v.x - x === 0; });
    };
    ...
    c3_chart_internal_fn.findClosest = function(values, pos) {
        ...
        values.filter(function(v) { return v && $$.isBarType(v.id); }).forEach(function(v) {
            var shape = $$.main
                .select("." + CLASS.bars + $$.getTargetSelectorSuffix(v.id) + " ." + CLASS.bar + "-" + v.index).node();
    ...
    c3_chart_internal_fn.convertDataToTargets = function(data, appendXs) {
        ...
        // set target types
        if (config.data_type) {
            $$.setTargetType($$.mapToIds(targets).filter(function(id) { return ! (id in config.data_types); }),
                config.data_type);
        }
                ...
                function() {
                    ...
                    if ($$.isBarType(closest.id) || $$.dist(closest, mouse) < 100) {
                        $$.main.selectAll("." + CLASS.shapes + $$.getTargetSelectorSuffix(closest.id))
                            .selectAll("." + CLASS.shape + "-" + closest.index).each(function() {
    ...
    c3_chart_internal_fn.updateTargetsForLine = function(targets) {
        ...
        targets.forEach(function(t) {
            $$.main.selectAll("." + CLASS.selectedCircles + $$.getTargetSelectorSuffix(t.id))
                .selectAll("." + CLASS.selectedCircle).each(function(d) {
    ...
    c3_chart_internal_fn.generateDrawLine = function(lineIndices, isSub) {
        ...
        if (!config.line_connectNull) {
            line = line.defined(function(d) { return d.value != null; });
        }
        return function(d) {
    ...
    c3_chart_internal_fn.generateGetAreaPoints =
    ...
    c3_chart_internal_fn.pointExpandedR = function(d) {
        ...
        return config.point_focus_expand_enabled
            ? (config.point_focus_expand_r ? config.point_focus_expand_r : $$.pointR(d) * 1.75)
            : $$.pointR(d);
    };
    ...
    c3_chart_internal_fn.updateText = function(durationForExit) {
        ...
        $$.mainText.enter().append("text")
            ...
            .attr("text-anchor",
                function(d) { return config.axis_rotated ? (d.value < 0 ? "end" : "start") : "middle"; })
            .style("stroke", "none")
    ...
    c3_chart_internal_fn.getTextRect = function(text, cls) {
        var dummy = this.d3.select("body").append("div").classed("c3", true),
            svg = dummy.append("svg").style("visibility", "hidden").style("position", "fixed").style("top", 0)
                .style("left", 0),
            rect;
        svg.selectAll(".dummy")
            .data([text])
    ...
    c3_chart_internal_fn.updateLegendWithDefaults = function() {
        ...
        $$.updateLegend($$.mapToIds($$.data.targets),
            { withTransform: false, withTransitionForTransform: false, withTransition: false });
    };
    ...
    Axis.prototype.getXAxis = function getXAxis(scale,
        orient,
        tickFormat,
        tickValues,
        withOuterTick,
    ...
    Axis.prototype.updateXAxisTickValues = function updateXAxisTickValues(targets, axis) {
        ...
        if (config.axis_x_tick_fit || config.axis_x_tick_count) {
            tickValues =
                this.generateTickValues($$.mapTargetsToUniqueXs(targets), config.axis_x_tick_count, $$.isTimeSeries());
        }
        if (axis) {
    ...
    Axis.prototype.getYAxis =
        function getYAxis(scale, orient, tickFormat, tickValues, withOuterTick, withoutTransition) {
            var axisParams = {
                    withOuterTick: withOuterTick,
                    withoutTransition: withoutTransition,
                },
    ...
    c3_chart_internal_fn.regionX = function(d) {
        ...
        if (d.axis === "y" || d.axis === "y2") {
            ...
            xPos = config.axis_rotated
                ? 0
                : ("start" in d ? $$.x($$.isTimeSeries() ? $$.parseDate(d.start) : d.start) : 0);
        }
        return xPos;
    };
    ...
    c3_chart_internal_fn.regionWidth = function(d) {
        ...
        if (d.axis === "y" || d.axis === "y2") {
                ...
                ? $$.width
                : ("end" in d ? $$.x($$.isTimeSeries() ? $$.parseDate(d.end) : d.end) : $$.width);
        }
    ...
    c3_chart_internal_fn.regionHeight = function(d) {
        ...
        if (d.axis === "y" || d.axis === "y2") {
                ...
                ? ("end" in d ? $$.x($$.isTimeSeries() ? $$.parseDate(d.end) : d.end) : $$.height)
                : $$.height;
        }
        return end < start ? 0 : end - start;
    };
        ...
        if (config.data_selection_enabled && config.data_selection_isselectable(d)) {
            if (!config.data_selection_multiple) {
                $$.main.selectAll("." +
                    ...
                    .selectAll("." + CLASS.shape).each(function(d, i) {
                        var shape = d3.select(this);
    ...
    c3_chart_internal_fn.redrawSubchart = function(withSubchart,
        transitions,
        duration,
        durationForExit,
        areaIndices,
    ...
    c3_chart_internal_fn.dataLabelFormat = function(targetId) {
        var $$ = this,
            ...
            format,
            defaultFormat = function(v) { return isValue(v) ? +v : ""; };
